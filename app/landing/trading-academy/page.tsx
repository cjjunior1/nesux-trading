...
// Botones CTA reutilizables (orden invertido: primero el asistente, luego WhatsApp).
function CtaButtons() {
  return (
    <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mt-8">
      {/* AZUL: abrir el asistente de trading */}
      <button
        type="button"
        onClick={() => { try { window.dispatchEvent(new CustomEvent("nx-open-chat")); } catch {} }}
        className="inline-flex items-center gap-2 rounded-full bg-blue-600 hover:bg-blue-700 text-white text-lg px-8 py-4 shadow-lg font-bold transition-all hover:shadow-xl"
      >
        💬 Hablar con el asistente
      </button>
      {/* VERDE: directo a WhatsApp (Nexy), sin pasar por el asistente */}
      <a
        href="https://wa.me/message/LE6U6V6QDCM4B1"
        target="_blank"
        rel="noopener noreferrer"
        className="inline-flex items-center gap-2 rounded-full bg-green-600 hover:bg-green-700 text-white text-lg px-8 py-4 shadow-lg font-bold transition-all hover:shadow-xl"
      >
        📲 Hablar por WhatsApp
      </a>
    </div>
...
export default function TradingAcademyLanding() {
  return (
    <main className="landing-page min-h-screen bg-gradient-to-br from-[var(--color-darker)] via-slate-900 to-[var(--color-dark)] text-white flex flex-col items-center">
      ...
      <section className="max-w-3xl w-full px-4 mx-auto pt-24 pb-10 text-center">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 max-w-2xl mx-auto mt-5 rounded-full bg-emerald-600 hover:bg-emerald-700 text-white text-xl md:text-2xl font-extrabold px-8 py-4 shadow-lg transition-all hover:shadow-xl"
        >
          🚀 Prueba nuestro bot en una cuenta demo GRATIS
        </a>
      ...
      <section className="max-w-3xl w-full mx-auto py-8 md:py-16 px-4 text-center">
        <div className="relative w-full max-w-2xl mx-auto mb-6 aspect-video rounded-2xl overflow-hidden shadow-lg bg-slate-900">
          <iframe
            className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-[116%]"
            src="https://www.youtube.com/embed/Q6Wr_oLkVnk?modestbranding=1&rel=0"
      ...
      {/* CTA FINAL */}
      <section className="max-w-3xl w-full mx-auto px-4 text-center pb-4">
        <h2 className="section-title mb-2">¿Listo para dar el paso?</h2>
        <CtaButtons />
      </section>
